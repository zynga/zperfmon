...
// Function to register a node in background and return the pid
function register_node($ip,$f5_web_client_port,$f5_partition,$f5_user,$f5_pass,$f5_pool,$f5_ip1,$f5_ip2=NULL){
        global $server_cfg;
        $command = "/usr/local/bin/register.py -a $ip -p $f5_web_client_port -n $f5_partition add $f5_user $f5_pass $f5_pool $f5_ip1 $f5_ip2";

        $target = sprintf($server_cfg['log_file'],$ip);

        if($f5_ip2)
                $command = $command." ".$f5_ip2;
	...
	$f5_credentials = $game_cfg["f5_credentials"][$pool_name];
		...
		echo "Registered ".count($ip_list)." nodes with F5 successfully... \n";
                exit(1);
	}
?>	
