 ...
 *
 * Resolve function names for call_user_func and call_user_func_array.
 */
...
#else
    current->mu_start_hprof  = 0; 
    current->pmu_start_hprof = 0;
#endif
  }
}
...
ZEND_DLEXPORT void hp_execute_internal(zend_execute_data *execute_data,
  ...
  // skipping the profilling of dispatcher functions like call_user_func,"call_user_func_array"
  if(func && (!strcmp(func,"call_user_func") || !strcmp(func,"call_user_func_array"))){
	efree(func);
	func = NULL;
	// if func = NULL profilling below will be turned off automatically.
  }
