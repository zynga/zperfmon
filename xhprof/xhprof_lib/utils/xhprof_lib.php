...
function xhprof_trim_run($raw_data, $functions_to_keep) {
  ...
  foreach($functions_to_keep as $func) {
    $function_map[$func] = 1;
  }
...
//
// Same as xhprof_aggregate_runs - excecpt the profiles are located in
// the directory "run_dir" instead of the pre-configured one. Needs
// the function get_run_fqpn() to be defined.
// 
function xhprof_aggregate_runs_fqpn($xhprof_runs_impl, $runs, $source, $run_dir)
{
  return xhprof_aggregate_runs($xhprof_runs_impl, $runs,
                               null, $source, false, $run_dir);
}

function xhprof_aggregate_runs_list($xhprof_runs_impl, $file_list)
{
  return xhprof_aggregate_runs($xhprof_runs_impl, $file_list,
                               null, null, false, ":file-name-list:");
}
...
function xhprof_aggregate_runs($xhprof_runs_impl, $runs,
                               $wts, $source="phprof",
                               $use_script_name=false, $run_dir=null) {
  ...
  foreach($runs as $idx => $run_id) {
    ...
    if (!$run_dir) {
      ...
      if ($run_dir == ":file-name-list:") {
	$raw_data = $xhprof_runs_impl->get_run_file($run_id, $description);
      } else {
	$raw_data = $xhprof_runs_impl->get_run_fqpn($run_id, $source, $description, $run_dir);
      }
    }
    ...
    if ($idx == 0) {
      foreach ($raw_data["main()"] as $metric => $val) {
        ...
        // update the metrics set after every profile to take the intersection of all profile metrics
        $metrics_tmp = array();
		foreach ($raw_data["main()"] as $metric => $val) {
			if (isset($val)) {
				$metrics_tmp[] = $metric;
		...
		$pop_metrics = array_diff($metrics, $metrics_tmp);
		$metrics = array_intersect($metrics, $metrics_tmp);
    }
	...
	foreach ($pop_metrics as $pop_metric) {
          unset($raw_data_total[$parent_child][$pop_metric]);
        }
      }
    }
  }
